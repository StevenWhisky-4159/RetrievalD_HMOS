#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""从 dataset.xlsx 固定随机抽样并运行 BM25 检索。"""
...
DEFAULT_OUTPUT = _HERE / "data" / "random_20_results.json"
...
def gold_rank(gold_path: str, results) -> int:
    ...
    for result in results:
        candidate = normalize_path(result.path)
        if candidate == normalized_gold or candidate.endswith("/" + normalized_gold):
            return result.rank
    return 0
...
def evaluate(
    ...
    *,
    ...
    path_prefix: str | None = DEFAULT_PATH_PREFIX,
) -> dict[str, object]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="随机抽样 20 条 BM25 检索评测")
